...
class Linear_ResBlock(BaseModule):
    ...
    def forward(self, x: SparseConvTensor) -> SparseConvTensor:#3月29日,sparseconvtensor forward,这里之后进行修改
        ...
        res.features = self.act0_0(res.features)
        res.features = self.bn0_0(res.features)

        res = self.conv0_1(res)
        res.features = self.act0_1(res.features)
        res.features = self.bn0_1(res.features)

        res = self.conv0_2(res)
        res.features = self.act0_2(res.features)
        res.features = self.bn0_2(res.features)


        return res


class Linear_DownBlock(BaseModule):
...
class Linear_UpBlock(BaseModule):
...
class Linear_withddcm_3DSpconv(BaseModule):
    ...
    def __init__(self,#关注重点
        ...
        self.down_context = Linear_ResBlock(#关注重点
