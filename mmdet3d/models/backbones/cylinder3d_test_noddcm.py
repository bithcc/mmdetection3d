...
class Base_ResBlock(BaseModule):
    ...
    def __init__(self,
        ...
        self.act_0= build_activation_layer(act_cfg)
        self.bn_0 = build_norm_layer(norm_cfg, out_channels)[1]

        self.conv_1 = SubMConv3d(
            in_channels,#16
            out_channels,#32
            kernel_size=(1, 1, 1),
            padding=0,
        ...
        self.bn_1 = build_norm_layer(norm_cfg, out_channels)[1]
    ...
    def forward(self, x: SparseConvTensor) -> SparseConvTensor:#3月29日,sparseconvtensor forward,这里之后进行修改
        ...
        shortcut.features = self.act_0(shortcut.features)
        shortcut.features = self.bn_0(shortcut.features)


        res = self.conv_1(x)
        res.features = self.act_1(res.features)
        res.features = self.bn_1(res.features)
...
class Base_DownBlock(BaseModule):
...
class Basenoddcm_3DSpconv(BaseModule):
    ...
    def __init__(self,#关注重点
        ...
        self.down_context = Base_ResBlock(#关注重点
